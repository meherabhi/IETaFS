# -*- coding: utf-8 -*-
"""
Created on Thu Mar 18 15:31:02 2021
...
def mainWarpField(im,srcPoints,dstPoints,dt_im, fake_batch, j):
...
real_batch = transforms.ToTensor()(Image.open('E:/Full_Model/Fine_Tune_data/Frames/Real_Frames/012130.jpg')).unsqueeze(0)
fake_batch = transforms.ToTensor()(Image.open('E:/Full_Model/Fine_Tune_data/Frames/Fake_Frames/012130.jpg')).unsqueeze(0)
fake_batch = fake_batch.requires_grad_(True)
print(fake_batch.shape)


batch_size, im_channels, im_height, im_width = real_batch.shape


for j in range(0, batch_size):
    
    frame= kornia.tensor_to_image(fake_batch[j,:,:,:])
    cv2.imwrite("real_Image.jpg",frame)

    
    # frame = real_batch[j,:,:,:].transpose(0,2).transpose(0,1).cpu().detach().numpy()*255
    # frame = frame.astype(np.uint8)
    
    # newRect = getFaceRect(frame, faceDetector)
    # landmarks_frame_init = landmarks2numpy(landmarkDetector(frame, newRect))
    
    # im = fake_batch[j,:,:,:].transpose(0,2).transpose(0,1).cpu().detach().numpy()*255
...
#     im_new = mainWarpField(im,srcPoints,dstPoints,dt_im, fake_batch,j) 

# im = fake_batch[0,:,:,:].transpose(0,2).transpose(0,1).cpu().detach().numpy()*255
# im = im.astype(np.uint8)
# imgplot = plt.imshow(im)
# plt.show()
